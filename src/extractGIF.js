...
import "./extractGIF.css"


function ExtractGIF({handleOptionSelect, handleSetFileData, fillZeroes,
  formatTime, validateTime
}) {

  const gifQualities = ["240p", "360p", "480p"];
  ...
  const extractGIF = () => {
    ...
    fetch(
      `${BASE_URL}/api/extract-gif`,
      {
        method: "POST",
        body: JSON.stringify({
          fileName: modifiedFileName,
          startDuration: fillZeroes(range.from),
          endDuration: fillZeroes(range.to),
          gifQuality: gifQualities[currentQualityIndex].slice(0,-1)
        }),
        headers: {
      ...
      fetch(
        ...
        if(progressState === 'progress') {
          ...
          setTimeout(() => {
            ...
            // SEND DATA TO extract-gif ROUTE AFTER FILE HAS BEEN UPLOADED
            // CALL extractGIF() FUNCTION
            extractGIF()
          }, 500)
        }
  ...
  return (
    <div>
      <div className='extract-gif-title'>
            <div className='back-btn' onClick={() => handleOptionSelect("Options")}>
              <i className='fa-solid fa-angle-left'></i>
            </div>
            <div>
              <span>Extract GIF</span>
            </div>
      </div>
      <div className='extract-gif-body'>
            ...
            <div className='quality'>
                <div className='quality-title'>
                    <p>Select GIF quality</p>
                </div>
                <div className='select-quality'>
                    <div className='current-quality'>
                      {gifQualities[currentQualityIndex]}
                    </div>
                    <div className='change-quality'>
                        <button className='decrease-quality'
                        onClick={() => {
                        ...
                        <button className='increase-quality'
                        onClick={() => {
                          setCurrentQualityIndex((prev) => prev + 1)
                        }}
                        disabled={currentQualityIndex === (gifQualities.length - 1)}>
            ...
            <div className='note-section'>
              <span>*</span>
              <p>Output GIF will run at 20 FPS</p>
            </div>
            <div className='note-section'>
              <span>*</span>
              <p>Width of the GIF will be determined automatically</p>
            </div>
            <div className='note-section'>
              <span>*</span>
              <p>Maximum length of GIF can be 6 seconds</p>
            </div>
